document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderMenu() {
        for (const cat in menu) {
            ...
            menu[cat].forEach((item, index) => {
                const isInitialItem = initialMenuData[cat] && 
                                      initialMenuData[cat].some(initialItem => initialItem.name === item.name);
                
                
                if (isInitialItem) return;
                ...
                card.innerHTML = `
                    ...
                    <button class="edit-btn" data-category="${cat}" data-index="${index}">EDIT</button>
    ...
    function attachListeners() {
        ...
        document.querySelectorAll(".edit-btn").forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                const itemToEdit = menu[cat][index];
                ...
                document.querySelector(".manager-controls").scrollIntoView({ behavior: 'smooth' });
            });
        });
    }
    ...
    addBtn.addEventListener("click", () => {
        ...
        if (editingIndex !== -1 && editingCategory === targetCategory) {
            
            menu[editingCategory][editingIndex] = item;
